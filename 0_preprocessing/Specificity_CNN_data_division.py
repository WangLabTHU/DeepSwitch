...
def prepare_data(th):
    ...
    data = data[(data['Specificity'] <= specificity_median) | (data['Specificity'] >= th)]
    ...
    # Generate dataset
    train_dataset = Generate_input(train_data, th)
    val_dataset = Generate_input(val_data, th)
    test_dataset = Generate_input(test_data, th)
...
def main():
    # Dataset division threshold / 5%, 10%, 15%, 20%, 25%, 30%
    thresholds = [0.3333333333333333, 0.3157306298987557, 0.28988284893299654,
                  0.26124629377247005, 0.2327652759094884, 0.20590631247054375]
    repeated_values = [val for val in thresholds for _ in range(3)]
    auROCs = []

    # Model evaluation with different thresholds
    for th in repeated_values:
        print('threshold: ', th)

        # Dataset load
        train_dataset, val_dataset, test_dataset = prepare_data(th)
        train_loader = DataLoader(train_dataset, batch_size=256, shuffle=True)
        val_loader = DataLoader(val_dataset, batch_size=256, shuffle=False)
        test_loader = DataLoader(test_dataset, batch_size=256, shuffle=False)

        # Model initialization
        model = CNNModel(seq_len=100, kernel_sizes=5)
        model.to(device)
    ...
    # auROC save to csv
    auROCs = np.array(auROCs).reshape(len(thresholds), 3)
    df = pd.DataFrame(auROCs, columns=['Repeat1', 'Repeat2', 'Repeat3'])
    df.to_csv('Evaluations/Specificity_auROC_different_data_division_thresholds.csv', index=False)
