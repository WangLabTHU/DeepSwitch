...
def Generate_input(pre_data, pos_size):
    ...
    labels = pre_data['Strength'].tolist()
...
def prepare_data(pos_size):
    ...
    specificity_median = data['Strength'].median()
    data = data[(data['Strength'] <= specificity_median) | (data['Strength'] >= 66.98958249899084)]
...
def main():
    ...
    df.to_csv('Evaluations/Strength_auROC_different_window_pos_size.csv', index=False)
