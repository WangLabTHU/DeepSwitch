...
def prepare_data(th):
    ...
    strength_median = data['Strength'].median()
    data = data[(data['Strength'] <= strength_median) | (data['Strength'] >= th)]
...
def main():
    # Dataset division threshold / 1%, 5%, 10%, 15%, 20%, 25%, 30%
    thresholds = [66.98958249899084, 20.67225970818582, 12.769495368493274,
                  9.533796501201364, 7.800976574016678, 6.6130579077021965, 5.763844182561292]
    ...
    df.to_csv('Evaluations/Strength_auROC_different_data_division_thresholds.csv', index=False)
