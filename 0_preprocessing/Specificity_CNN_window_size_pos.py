...
# Dataset generation
def Generate_input(pre_data, pos_size):
    # dataset preprocessing
    sequences = pre_data['Sequences'].tolist()
    sequences = [seq[len(seq) // 2 + pos_size[0]: len(seq) // 2 + pos_size[0] + pos_size[1]] for seq in sequences]
    labels = pre_data['Specificity'].tolist()
...
# Dataset Division
def prepare_data(pos_size):
    data = pd.read_csv('DeepCP_dataset/switch_100bp.csv')
    specificity_median = data['Specificity'].median()
    data = data[(data['Specificity'] <= specificity_median) | (data['Specificity'] >= 0.3157306298987557)]
    ...
    # Generate dataset
    train_dataset = Generate_input(train_data, pos_size)
    val_dataset = Generate_input(val_data, pos_size)
    test_dataset = Generate_input(test_data, pos_size)
...
def evaluate_model(model, criterion, data_loader):
    ...
    return average_loss, accuracy, precision, recall, f1, auc_roc
...
def main():
    Win_pos_size = [[-50, 100],
                    [-50, 90], [-40, 90],
                    [-50, 80], [-40, 80], [-30, 80],
                    [-50, 70], [-40, 70], [-30, 70], [-20, 70],
                    [-50, 60], [-40, 60], [-30, 60], [-20, 60], [-10, 60],
                    [-50, 50], [-40, 50], [-30, 50], [-20, 50], [-10, 50], [0, 50],
                    [-50, 40], [-40, 40], [-30, 40], [-20, 40], [-10, 40], [0, 40], [10, 40]]
    Win_pos_size = [elem for elem in Win_pos_size for _ in range(3)]

    auROCs = []
    for pos_size in Win_pos_size:
        # Dataset load
        train_dataset, val_dataset, test_dataset = prepare_data(pos_size)
        train_loader = DataLoader(train_dataset, batch_size=256, shuffle=True)
        val_loader = DataLoader(val_dataset, batch_size=256, shuffle=False)
        test_loader = DataLoader(test_dataset, batch_size=256, shuffle=False)

        # Model initialization
        model = CNNModel(seq_len=pos_size[1], kernel_sizes=5)
        model.to(device)
        ...
        for epoch in range(num_epochs):
            ...
            # Model validation
            average_loss, accuracy, precision, recall, f1, auc_roc = evaluate_model(model, criterion, val_loader)
        ...
        average_loss, accuracy, precision, recall, f1, auc_roc = evaluate_model(model, criterion, test_loader)
        ...
        auROCs.append(auc_roc)

    # auROC save to csv
    Win_pos_size = np.array(Win_pos_size).T
    df = pd.DataFrame({'auROCs': auROCs, 'Position': Win_pos_size[0], 'Size': Win_pos_size[1]})
    df.to_csv('Evaluations/Specificity_auROC_different_window_pos_size.csv', index=False)
